...
#[repr(C)]
pub struct IsoMCanonCmp1Box {
  pub headerSize: u16,
  pub version: u16,
  pub versionSub: u16, // Always 00 00
  pub f_width: u32,
  pub f_height: u32,
  pub tileWidth: u32,
  pub tileHeight: u32,
...
#[link(name = "crxdec")]
extern "C" {
  fn decompress_crx(cmp1: *const IsoMCanonCmp1Box, input: *const u16, input_len: size_t, output: *mut u16, output_len: size_t) -> c_int;
}

pub fn decompress_compat(input: &[u8], cmp1: &Cmp1Box) -> Vec<u16> {
  let mut out: Vec<u16> = Vec::new();
  out.resize((cmp1.f_height * cmp1.f_width) as usize, 0);

  let cmp1_compat = IsoMCanonCmp1Box {
    headerSize: cmp1.header_size,
    version: cmp1.version,
    versionSub: cmp1.version_sub,
    f_width: cmp1.f_width,
    f_height: cmp1.f_height,
    tileWidth: cmp1.tile_width,
    tileHeight: cmp1.tile_height,
    nBits: cmp1.n_bits,
    nPlanes: cmp1.n_planes,
    cfaLayout: cmp1.cfa_layout,
    encType: cmp1.enc_type,
    imageLevels: cmp1.image_levels,
    hasTileCols: cmp1.has_tile_cols,
    hasTileRows: cmp1.has_tile_rows,
    mdatHdrSize: cmp1.mdat_hdr_size,
  };

  unsafe {
    let code = decompress_crx(&cmp1_compat, input.as_ptr() as *const u16, input.len(), out.as_mut_ptr(), out.len() * 2); // sizeof(u16)
    assert!(code == 0);
  }
