...
pub fn compress_original(uncomp_data: &[u8]) -> Result<Vec<u8>, std::io::Error> {
  let pool = rayon::ThreadPoolBuilder::new().num_threads(rayon::current_num_threads()/2+1).build().unwrap();
