  ...
  W: Write + Seek + Send,
{
  ...
  let mut rawimage = decoder.raw_image(rawfile, raw_params.clone(), false)?;
  ...
  if params.preview || params.thumbnail {
    match generate_preview(rawfile, decoder.as_ref(), &rawimage) {
      Ok(image) => {
        if params.preview {
...
fn generate_preview(rawfile: &RawSource, decoder: &dyn Decoder, rawimage: &RawImage) -> crate::Result<DynamicImage> {
