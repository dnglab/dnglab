...
Register-ArgumentCompleter -Native -CommandName 'dnglab' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        ...
        'dnglab;convert' {
            ...
            [CompletionResult]::new('--jxl-distance', '--jxl-distance', [CompletionResultType]::ParameterName, 'JPEG XL distance (0.0 = lossless, 1.0 = visually lossless)')
            [CompletionResult]::new('--jxl-effort', '--jxl-effort', [CompletionResultType]::ParameterName, 'JPEG XL encoding effort (1 = fast, 10 = best quality)')
            [CompletionResult]::new('--jxl-decode-speed', '--jxl-decode-speed', [CompletionResultType]::ParameterName, 'JPEG XL decode speed hint (1 = slow, 4 = fast)')
